...
function initializeGmap( mapId, mapIndex ) {
	...
	if ( $( "#wpsl-gmap" ).length ) {
		 ...
		 */
		if ( $( ".wpsl-dropdown" ).length && wpslSettings.enableStyledDropdowns == 1 ) {
			//createDropdowns();						
			createStateDropdowns();
			createCityDropdowns();
			createAreaDropdowns();
		} else {
...
function collectAjaxData( startLatLng, resetMap, autoLoad ) {
	...
	*/
    ...
    if ( typeof $("#pagloc").val() !== "undefined" ) {
		ajaxData.pagelocation = $( "#pagloc" ).val();
	}
	return ajaxData;
}
...
function addMarker( latLng, storeId, infoWindowData, draggable, infoWindow ) {
    ...
    google.maps.event.addListener( marker, "mouseover",( function( currentMap ) {
		return function() {
...
function createStateDropdowns() {
	
	var maxDropdownHeight = Number( wpslSettings.maxDropdownHeight );
		
	var $this = $( "#wpsl-states .wpsl-dropdown" );
	var	active, maxHeight;
	...
	$this.$dropdownItem.on( "click", function( e ) {
		...
		wpsl_state = $this.$dropdownWrap.find( $( "#wpsl_state" ) );
		if(wpsl_state.length > 0) {
			$.ajax({
		      url: naturals_object.ajax_url+'?action=getcity',
		      data: 'state='+$( this ).attr( "data-value" ),
		      type: 'post',
		      success: function (data) {
		      	$("#wpsl-cities").replaceWith(data);
		      	createCityDropdowns();
		      }
		  });
		}
...
function createCityDropdowns() {
	
	var maxDropdownHeight = Number( wpslSettings.maxDropdownHeight );
		
	var $this = $( "#wpsl-cities .wpsl-dropdown" );
	var	active, maxHeight;
	...
	$this.$dropdownItem.on( "click", function( e ) {
		...
		wpsl_city = $this.$dropdownWrap.find( $( "#wpsl_city" ) );
		if(wpsl_city.length > 0) {
			$.ajax({
		      url: naturals_object.ajax_url+'?action=getarea',
		      data: 'city='+$( this ).attr( "data-value" ),
		      type: 'post',
		      success: function (data) {
		      	$("#wpsl-areas").replaceWith(data);
		      	createAreaDropdowns();
		      }
		  });
		}
...
function createAreaDropdowns() {
	
	var maxDropdownHeight = Number( wpslSettings.maxDropdownHeight );
		
	var $this = $( "#wpsl-areas .wpsl-dropdown" );
	var	active, maxHeight;
